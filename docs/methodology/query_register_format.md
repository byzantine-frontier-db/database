...
**Rule 1 — a recorded prediction stays legible, in both fields.** The register is a prospective record, and its worth is that it cannot quietly improve its own track record once a result is known. A prediction, here, is any recorded claim with truth-conditions, because only a claim that can turn out wrong is part of a track record. The commitment has two halves, one per field.

- **(a) Field 3 is immutable.** Once an entry is Approved, field 3 may not be edited. Corrections are appended as dated amendments below the original, which remains visible. The guarantee is structural: editing is made impossible rather than merely discouraged.
- **(b) A superseded field-4 prediction is preserved, not overwritten.** Field 4 is amendable, and correcting it is how the register learns. But where a correction supersedes a claim already recorded in field 4, the superseded claim stays visible and the correction is appended below it, dated and attributed to the run, review, or decision that occasioned it. A field-4 limitation with truth-conditions — that a count is a lower bound, that a flag errs in one direction — carries the same prospective value as field 3 and is preserved the same way. This governs supersession only: a genuinely new limitation supersedes nothing and is added freely, and a copy-edit that changes no claim triggers nothing.

*Rationale.* Field-3 immutability is enforced by rule. Field-4 legibility, until now, rested on whoever wrote the amendment remembering to keep the falsified claim visible, and that discretion has failed in practice, as the QR-402 mediation correction of 2026-07-23 showed. The two are one commitment at two levels: a register free to rewrite either field's predictions after the fact is worthless as a prospective record. The line is truth-conditional because only a truth-conditional claim can be falsified, and so only such a claim is a track-record event. A change to a limitation's severity or scope that flips no truth-condition — "will miss cases" softened to "may occasionally miss cases" — is presentation, not correction, and sits outside the rule. That hedging is nonetheless a real way a register launders a bad prediction, and is for a reviewer to notice and challenge, not for this rule to define.
